...
import { PlatformStackPicker, type PlatformStackValue } from "@/components/platform-stack-picker";
import { RoleMap, type RoleMapStack } from "@/components/role-map";
import { saveProjectAction } from "./actions";

/**
 * Composition canvas (client): platform stack picker + tiered recommendations
 * on the left, live coverage + role map + dashboard set + save form in a
 * sticky right rail. All props are plain JSON — selection state lives here;
 * coverage and recommendations recompute on every toggle with the same pure
 * engine the server uses. Selections autosave to localStorage per sector ×
 * scenario and restore on return.
 */
...
export function ComposerCanvas({
  ...
  scenario: Scenario;
  elements: Element[];
  dashboardPool: Dashboard[];
  bestPractices: BestPractice[];
  obligations: Obligation[];
  platforms: Platform[];
  frictionPatterns: FrictionPattern[];
  packCodes: Record<string, string>;
  defaultPlatformKeys: string[];
}) {
  const sectorKey = sector.key;
  const sectorName = sector.name;

  // ---------- Platform stack (primary + market variants) ----------
  const [platformStack, setPlatformStack] = useState<PlatformStackValue>({
    platformKeys: defaultPlatformKeys,
    variants: [],
  });

  const effectivePlatformKeys = useMemo(
    () => unionPlatformKeys(platformStack.platformKeys, platformStack.variants),
    [platformStack],
  );
  ...
  const ctx: EngineContext = useMemo(
    () => ({
      ...
      scenarios: [scenario],
      elements,
      dashboards: dashboardPool,
      bestPractices,
      obligations,
      platforms,
      frictionPatterns,
    }),
    [sector, scenario, elements, dashboardPool, bestPractices, obligations, platforms, frictionPatterns],
  );
  ...
  const composeInput = useMemo(
    () => ({
      ...
      platformKeys: effectivePlatformKeys as PlatformKey[],
    }),
    [sector.key, scenario.key, effectivePlatformKeys],
  );

  const recommendations = useMemo(
    () => recommendElements(composeInput, ctx),
    [composeInput, ctx],
  );
  const dashboards = useMemo(
    () => recommendDashboards(composeInput, ctx).slice(0, 6),
    [composeInput, ctx],
  );

  // Role map: one report per stack, run independently (not on the union) —
  // primary always, plus one per named market variant.
  const roleMapStacks: RoleMapStack[] = useMemo(() => {
    const stacks: RoleMapStack[] = [
      {
        label: "Primary stack",
        report: computeRoleMap(
          { platformKeys: platformStack.platformKeys as PlatformKey[] },
          { platforms, frictionPatterns },
        ),
      },
    ];
    platformStack.variants.forEach((variant, i) => {
      stacks.push({
    ...
    return stacks;
  }, [platformStack, platforms, frictionPatterns]);
    ...
    <div className="space-y-6">
      <Card>
        <CardContent className="pt-5">
          <PlatformStackPicker platforms={platforms} value={platformStack} onChange={setPlatformStack} />
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        ...
        {/* Right rail: coverage, role map, dashboards, save */}
        <div className="space-y-4 self-start lg:sticky lg:top-20">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Platform role map</CardTitle>
              <p className="text-xs text-slate-500">How the selected stack divides capability work.</p>
            </CardHeader>
            <CardContent>
            ...
            <CardContent>
              ...
              >
                ...
                <input
                  type="hidden"
                  name="platformKeys"
                  value={JSON.stringify(platformStack.platformKeys)}
                />
                <input
                  type="hidden"
                  name="platformVariants"
                  value={JSON.stringify(platformStack.variants)}
                />
