...
const SEVERITY_META = {
  ...
  serious: { label: "Serious", color: "var(--status-serious)", Icon: TriangleAlert },
  warning: { label: "Warning", color: "var(--status-warning)", Icon: AlertTriangle },
} as const;
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {dq.severityTotals.map(({ severity, count }) => {
          ...
          return (
            <div
              key={severity}
              className="flex items-center gap-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 px-5 py-4"
            >
              <meta.Icon className="h-6 w-6 shrink-0" style={{ color: meta.color }} />
              <div>
                <p className="text-2xl font-semibold text-slate-900 dark:text-white tabular-nums">{count}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{meta.label} breaches open</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {dq.ownerAging.map((b) => (
                <div key={b.bucket} className="grid grid-cols-[90px_1fr_40px] items-center gap-3">
                  <span className="text-xs text-slate-600 dark:text-slate-300">{b.bucket}</span>
                  <div className="h-3 overflow-hidden rounded-sm bg-slate-200 dark:bg-slate-800/80">
                    <div
            ...
            <dl className="mt-5 grid grid-cols-3 gap-3 border-t border-slate-200 dark:border-slate-800 pt-4 text-center">
              <div>
            ...
            <table className="w-full min-w-[560px] text-sm">
              ...
              <tbody>
                {dq.reportInputsAtRisk.map((r) => {
                  ...
                  return (
                    <tr key={r.name} className="border-b border-slate-200/70 dark:border-slate-800/60">
                      <td className="py-2 pr-4 text-slate-700 dark:text-slate-200">{r.name}</td>
                      <td className="py-2 pr-4">
                        <span className="inline-flex items-center gap-1.5 text-xs" style={{ color: meta.color }}>
                          <meta.Icon className="h-3.5 w-3.5" /> {meta.label}
                        </span>
                      </td>
                      <td className="py-2 pr-4 tabular-nums text-slate-600 dark:text-slate-300">{r.openBreaches}</td>
                      <td className="py-2 pr-4 tabular-nums text-slate-600 dark:text-slate-300">{r.daysOpen}</td>
                      <td className="py-2 text-slate-500 dark:text-slate-400">{r.owner}</td>
                    </tr>
                  );
                })}
              </tbody>
