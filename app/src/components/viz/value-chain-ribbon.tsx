          ...
          return (
            <button
              ...
              className={`relative flex min-w-28 flex-1 items-center justify-center px-3 py-2.5 text-xs font-medium transition
                ${isActive ? "bg-teal-500 text-slate-950" : "bg-slate-800/80 text-slate-300 hover:bg-slate-700"}
          ...
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {stage.dataDomains.map((d) => (
                <span key={d} className="rounded-full bg-teal-500/10 px-2 py-0.5 text-[11px] text-teal-300">
                  {d}
                </span>
              ))}
            </div>
