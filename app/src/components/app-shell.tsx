...
export async function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      {/* Sidebar */}
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-60 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 md:flex">
        ...
        <div className="border-t border-slate-200 dark:border-slate-800 px-5 py-4">
          <p className="truncate text-sm font-medium text-slate-700 dark:text-slate-200">{session.user.name}</p>
        ...
        <header className="sticky top-0 z-40 border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-950/90 backdrop-blur">
          <div className="flex items-center justify-between gap-4 px-4 py-3 sm:px-6">
            <div className="flex min-w-0 items-center gap-3">
              ...
              >
                <span className="text-xs font-semibold uppercase tracking-[0.25em] text-teal-600 dark:text-teal-400">
                  Artizent
                </span>
                <span className="font-serif text-lg text-slate-900 dark:text-white">Astra Velocity</span>
              </Link>
            </div>
            <div className="flex shrink-0 items-center gap-3">
              ...
              <form action={doSignOut}>
                <button
                  type="submit"
                  className="rounded-full border border-slate-300 dark:border-slate-700 px-3 py-1.5 text-sm text-slate-600 dark:text-slate-300 hover:border-slate-400 dark:hover:border-slate-500"
                >
                  Sign out
                </button>
              </form>
          ...
          {persona !== role && (
            <div className="border-t border-amber-500/20 bg-amber-500/10 px-4 py-1.5 text-xs text-amber-800 dark:text-amber-200 sm:px-6">
