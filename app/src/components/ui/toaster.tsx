...
const VARIANT_STYLES: Record<ToastVariant, string> = {
  success: "border-teal-500/40 text-teal-200",
  error: "border-red-500/40 text-red-200",
  info: "border-slate-700 text-slate-200",
};

const VARIANT_ICONS: Record<ToastVariant, React.ReactNode> = {
  success: <CircleCheck className="h-4 w-4 shrink-0 text-teal-400" aria-hidden />,
  error: <TriangleAlert className="h-4 w-4 shrink-0 text-red-400" aria-hidden />,
  info: <Info className="h-4 w-4 shrink-0 text-slate-400" aria-hidden />,
};
  ...
  return (
    ...
    >
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`toast-in flex items-start gap-2.5 rounded-xl border bg-slate-900/95 px-3.5 py-3 text-sm shadow-lg shadow-slate-950/50 backdrop-blur ${VARIANT_STYLES[t.variant]}`}
        >
