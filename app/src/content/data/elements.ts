...
export const ELEMENTS: Element[] = [
  ...
  // ───────────────────────── Governance-as-Code Expansion (BigID, Immuta, Power BI, dbt) ─────────────────────────
  {
    key: "bigid-classifier-correlation-policy-as-code",
    packKey: "vp-03",
    type: "toolkit",
    name: "BigID Classifier & Correlation Policy as Code",
    pitch: "The classifier assignment, correlation config, and downstream tag propagation as one deployable policy definition — not a console click-through per environment.",
    description:
      "A declarative BigID policy definition covering classifier assignment across NPPI-financial, claims-medical-narrative, and producer-license categories, the correlation-engine subject-matching configuration that clusters a data subject's records across policy, claims, and producer-licensing sources, and the attribute-mapping block that pushes confidence-gated findings to the catalog and to Immuta as attributes rather than re-classifying downstream. Deployed the same way across dev, test, and prod so a tuned classifier policy is never re-entered by hand per environment.",
    soWhat:
      "The classifier and correlation tuning a privacy team fought for in one environment survives the next environment promotion unchanged, and the catalog and Immuta both inherit the same finding instead of drifting from three separately configured copies.",
    audience: ["Privacy partners", "Security engineers", "Governance platform engineers"],
    capabilities: ["classification", "access_policy"],
    dataDomains: ["claim", "party", "producer-distribution"],
    bestPracticeKeys: ["governance-as-code", "classify-once-propagate-everywhere"],
    obligationKeys: ["glba-nppi", "hipaa-phi"],
    sectorAffinity: {
      "pc-personal": 3,
      "pc-commercial": 2,
    ...
    scenarioAffinity: { "sensitive-data-unlock": 3, "greenfield-platform": 1 },
    effortSavedStewardWeeks: 1,
    toolTags: ["classification (BigID, platform-native)", "catalog-suite (Informatica CDGC, Collibra, Atlan)"],
    platformAffinity: { bigid: 3, immuta: 1 },
  },
  {
    key: "immuta-purpose-policy-as-code",
    packKey: "vp-03",
    type: "toolkit",
    name: "Immuta Purpose-Based Policy as Code",
    pitch: "A purpose-based masking policy as a versioned, testable definition — the pattern Immuta's own product is built around.",
    description:
      "A declarative Immuta policy definition contrasting the claims-handling and analytics purposes over claimant medical-narrative and financial fields: attribute-based masking and row-filter rules keyed off the purpose, a data-source scope naming the exact schema and table the policy applies to, and a regression test block asserting each purpose still resolves as expected after a change. The policy explicitly consumes nppi-medical and nppi-financial attributes propagated from BigID or catalog classification — it declares no sensitivity labels of its own.",
    soWhat:
      "Purpose-based access stops being a described intent reviewed once at design time and becomes a version-controlled definition every policy change runs through CI against, with the blast radius of a change visible before it merges.",
    audience: ["Security engineers", "Privacy partners", "Governance platform engineers"],
    capabilities: ["access_policy", "classification"],
    dataDomains: ["claim", "party"],
    bestPracticeKeys: ["governance-as-code", "purpose-based-access"],
    obligationKeys: ["glba-nppi", "hipaa-phi"],
    sectorAffinity: {
      "pc-personal": 2,
      "pc-commercial": 2,
  ...
  {
    key: "power-bi-deployment-pipeline-certification-as-code",
    packKey: "vp-03",
    type: "toolkit",
    name: "Power BI Deployment Pipeline & Dataset Certification as Code",
    pitch: "Dev-to-prod promotion gated on a certified-glossary match and a governance sign-off, defined as pipeline stages, not a manual publish click.",
    description:
      "A deployment pipeline definition covering the dev, test, and prod stages a Power BI dataset moves through: a test-stage gate requiring every measure to match the certified glossary and RLS roles to reconcile against the upstream warehouse row-access policy, and a prod-stage gate requiring a logged governance sign-off before the certification badge and endorsement are applied on promotion. RLS role definitions map to warehouse domain tags rather than a separately maintained Power BI list, and each certified measure carries a recertify-on-formula-change trigger.",
    soWhat:
      "A dataset can't reach the certified badge by an author clicking publish from Desktop — certification becomes a pipeline outcome with a sign-off trail an audit can replay, not a status someone remembers to apply.",
    audience: ["BI developers", "Governance platform engineers", "Data stewards"],
    capabilities: ["semantic_layer", "access_policy", "stewardship_ops"],
    bestPracticeKeys: ["governance-as-code", "certified-data-products"],
    sectorAffinity: {
      "pc-personal": 2,
      "pc-commercial": 2,
      "life-annuities": 2,
    ...
    scenarioAffinity: { "report-integrity": 3, "greenfield-platform": 1 },
    effortSavedStewardWeeks: 1,
    toolTags: ["bi (Power BI, Tableau)", "ci-cd (Git-based pipeline, policy validation)"],
    platformAffinity: { "power-bi": 3, snowflake: 1 },
  },
  {
    key: "dbt-tests-contracts-as-code",
    packKey: "vp-03",
    type: "toolkit",
    name: "dbt Tests & Contracts as Code",
    pitch: "Column tests and an enforced contract on the certified premium model — the DAG stops shipping a breaking change silently.",
    description:
      "A dbt schema.yml pattern for the certified premium and claim fact models: a `contract: {enforced: true}` block that turns the model's column names and types into a breaking-change boundary for downstream consumers (the Power BI certified dataset, the Cortex semantic view), paired with column-level data_tests — not_null, accepted_values, a relationships test back to the policy dimension, and a custom earned-premium-reconciliation test expressing the same tolerance check the CDGC CDQ rule enforces elsewhere in the estate. Written in real dbt YAML syntax, ready to drop into a models/ directory.",
    soWhat:
      "A model change that would silently break the certified Power BI dataset or the Cortex semantic view now fails the dbt build instead — the contract and tests catch it in CI, before it reaches a report a business user trusts.",
    audience: ["Analytics engineers", "Data engineers", "DQ analysts"],
    capabilities: ["data_quality"],
    dataDomains: ["premium", "claim"],
    bestPracticeKeys: ["governance-as-code", "cde-anchored-quality"],
    obligationKeys: ["asop-23"],
    sectorAffinity: {
      "pc-personal": 2,
      "pc-commercial": 2,
    ...
    scenarioAffinity: { "report-integrity": 2, "financial-reconciliation": 2, "greenfield-platform": 1 },
    toolTags: ["dq-engine (Informatica CDQ, platform-native)", "warehouse (Snowflake, Databricks)"],
    platformAffinity: { dbt: 3 },
  },
];
